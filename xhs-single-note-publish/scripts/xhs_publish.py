...
def _generate_cover_with_codex(title: str, source_image: Path, output_path: Path) -> Path | None:
    ...
    prompt = (
        "$imagegen\n\n"
        f"Image 1 (input): 原小红书封面参考图\n\n"
        "---\n\n"
        "Task:\n"
        "基于 Image 1 的构图、版式、配色和视觉风格，仿写一张新的小红书笔记封面。\n"
        "保留原图的主题方向和整体风格气质，但重新设计为一款全新的封面。"
        "不要逐像素复制原图。\n\n"
        "---\n\n"
        "Use case:\n"
        "小红书手机信息流封面（Xiaohongshu mobile feed cover）\n"
        "竖版 3:4 构图（vertical 3:4 composition）\n"
        "面向手机屏幕缩略图阅读优化\n\n"
        "---\n\n"
        "Main title（EXACT，只渲染一次，一字不差，不加多余字符）:\n"
        f"{json.dumps(clean_title, ensure_ascii=False)}\n\n"
        "---\n\n"
        "Text requirements:\n"
        "- 超大字粗体中文字体，标题本身就是第一视觉焦点\n"
        "- 高对比度，缩略图尺寸也能清晰阅读\n"
        "- 自动分行 2～4 行，每行信息完整\n"
        "- 不加额外小字、不重复标题、不出现英文乱码\n\n"
        "---\n\n"
        "Layout:\n"
        "- 一个清晰的主体图像 + 一个主导标题块\n"
        "- 干净的层次、充足的留白、简洁的背景\n"
        "- 更像博主自制封面/知识卡片风格，而非通用广告海报\n\n"
        "---\n\n"
        "Preserve from Image 1:\n"
        "整体风格方向、色彩倾向、信息密度、主体类别、版式节奏\n\n"
        "Change:\n"
        "将原封面文字替换为上方指定的新标题，并重新设计封面布局\n\n"
        "---\n\n"
        "Constraints:\n"
        "无水印、无logo、无UI元素、无杂乱装饰、无密集段落文字、"
        "无多余字符、无不相干的装饰性文字\n\n"
        "---\n\n"
        "Quality & fidelity:\n"
        "medium/high quality (gpt-image-2 recommended)\n"
        "中文文字应当完整、准确、无错别字、无字形断裂\n\n"
        "---\n\n"
        f"直接保存为 \"{save_path}\""
    )
